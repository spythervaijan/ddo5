...
async def sender(tab_id, args, messages, context, page):
    ...
    while True:
        ...
        if elapsed >= 60:
            try:
                print(f"Tab {tab_id} switching to new tab after {elapsed:.1f}s")
                # Same URL ka fresh tab (new tab open + old tab close), kahin aur nahi jayega
                old_page = current_page
                new_page = await context.new_page()
                init_success = await init_page(new_page, old_page.url, dm_selector)
                if not init_success:
                    await new_page.close()
                    raise Exception(f"Tab {tab_id} new tab initialization failed after 3 tries")
                await old_page.close()
                current_page = new_page
                print(f"Tab {tab_id} successfully switched to new tab (old closed)")
            except Exception as switch_e:
                print(f"Tab {tab_id} tab switch failed after {elapsed:.1f}s: {switch_e}")
                raise Exception(f"Tab {tab_id} tab switch failed: {switch_e}")
            cycle_start = time.time()
            continue
